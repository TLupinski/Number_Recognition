...
class RecurrentModel(Recurrent):
    ...
    def __init__(self, input, output, initial_states=None, final_states=None, readout_input=None, teacher_force=False, decode=False, output_length=None, return_states=False, return_all_states=False, state_initializer=None, **kwargs):
        inputs = [input]
        outputs = [output]
        state_spec = None
    ...
    def __call__(self, inputs, initial_state=None, initial_readout=None, previous_state=None, ground_truth=None, **kwargs):
        ...
        if len(inputs) <= 2:
            if initial_state is not None:
                ...
                if previous_state is not None:
                    inputs.append(previous_state)
                    i = -1
                else:
                    i = 0
                if self.readout:
                    initial_state = self._get_optional_input_placeholder('initial_state', self.num_states - 1 + i)
                else:
                    initial_state = self._get_optional_input_placeholder('initial_state', self.num_states + i)
    ...
    def compute_output_shape(self, input_shape):
        ...
        if self.return_states and len(self.states) > 0:
            if self.return_all_states:
                mout = list(map(K.int_shape,self.model.output[1:]))
                for i in range(len(mout)):
                    mout[i] = (mout[i][0],) + (7,) + (mout[i][1],)
                output_shape = [output_shape] + mout
            else:    
...
class RecurrentSequential(RecurrentModel):
    ...
    def __init__(self, state_sync=False, decode=False, output_length=None, return_states=False, return_all_states=False, readout=False, readout_activation='linear', teacher_force=False, state_initializer=None, **kwargs):
    ...
    def build(self, input_shape):
        ...
        if self.readout:
            ...
            if self.readout in [True, 'add']:
                ...
                input_readout_merged = input
                initial_states = [readout]
            initial_states = initial_states + [Input(batch_shape=K.int_shape(s)) for s in initial_states]
